...
var showError = function (field, error) {
	...
	field.classList.add('regform__input--error');
	...
	if (field.type === 'radio' && field.name) {
		...
		if (group.length > 0) {
			for (var i = 0; i < group.length; i++) {
				group[i].classList.add('regform__input--error');
			}
	...
	// If not, create one
	var message = field.form.querySelector('.regform__error#error-for-' + id );
	if (!message) {
		message = document.createElement('div');
		message.className = 'regform__error';
...
var removeError = function (field) {
	...
	field.classList.remove('regform__input--error');
	...
	if (field.type === 'radio' && field.name) {
		...
		if (group.length > 0) {
			for (var i = 0; i < group.length; i++) {
				group[i].classList.remove('regform__input--error');
			}
			field = group[group.length - 1];
		}
	}
...
window.displayMailChimpStatus = function (data) {
	...
	if (data.result === 'error') {
		mcStatus.classList.remove('regform__success');
		mcStatus.classList.add('regform__error');
		return;
	}

	// Otherwise, add success class
	mcStatus.classList.remove('regform__error');
	mcStatus.classList.add('regform__success');
};
